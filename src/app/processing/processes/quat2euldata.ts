...
export class Quat2EulData extends AbstractDataBlock {
    ...
    public static convertToWorldReference(vector: Array<number>, quaternion: Array<number>): Array<number> {
        const correctedVector: Array<number> = Quat2EulData.toWorldReference(vector, quaternion);
    ...
    private static toWorldReference(vector: Array<number>, quaternion: Array<number>): Array<number> {
    ...
    private static quaternionConjugate(q: Array<number>): Array<number> {
        const output: Array<number> = [];
        output.push(q[0]);
        output.push(-q[1]);
        output.push(-q[2]);
    ...
    private static hamiltonian(q: Array<number>, r: Array<number>): Array<number> {
        const output: Array<number> = [];
    ...
    public static toEuler(q: Array<number>): Array<number> {
    ...
    private add(data: Data): Data {
        ...
        const accelInWorldFrame = Quat2EulData.convertToWorldReference(accelInBoardFrame, this.quaternion);
        const angularVelocityInWorldFrame = Quat2EulData.convertToWorldReference(angularVelocityInBoardFrame, this.quaternion);
